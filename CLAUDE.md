...
- **user_settings**: id (single row, id=1), first_name, last_name, email, address1, address2, city, state, phone, venmo, cashapp, paypal, zelle, show_earnings_on_timer, resume_window_minutes (default 60), default_rate (nullable decimal — user-level fallback hourly rate; pre-fills the new-project form on ProjectsPage and is used as the final rate fallback in amount calcs when both `time_entries.rate_override` and `projects.default_rate` are null/0)
- **invoices** stores `consolidated` (boolean, default `false`) captured from `CreateInvoiceInput` at creation time. When `false`, each hourly time entry becomes its own line item (`time_entry_id` set). When `true`, hourly entries are rolled up to one line per `(project_id, effective_rate)` with `time_entry_id=NULL`; the description is `"<Project Name>\n<Date1>, <Date2>, ..."` listing every unique date worked (chronological, deduplicated). `quantity` holds the summed hours and `rate` the single per-group rate. `unbillTimeEntry` re-derives consolidated lines after deleting the entry's own row, via `rebuildConsolidatedLineItems`. Flat-amount entries are never consolidated. The old `user_settings.consolidate_hours` global toggle was removed in migration `20260515000001`; older backups are normalized at restore (`backend/src/backup/restore.ts` maps `consolidate_hours=true → consolidated=true`). The frontend toggle is a checkbox in the Unbilled Time Entries card header on the Create Invoice page.

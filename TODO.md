...
- [x] Schedule + retention for backup destinations (`backup_destinations` gains `schedule_preset` ('off' | 'daily' | 'every3days' | 'weekly' | 'monthly' | 'yearly' | 'custom'), `schedule_cron`, `next_run_at`, `retention_days`, `retention_cron`, `retention_last_run_at`/`retention_last_error`/`retention_next_run_at` via migration `20260510000002_backup_schedule_retention`). `backend/src/backup/scheduler.ts` runs an in-process tick every 60s (started from `index.ts`), parses cron via `cron-parser`, runs due backups + retention deletes (older than `retention_days * 86400_000` based on remote `last_modified`), then recomputes `next_run_at` / `retention_next_run_at`. `BackupScheduleInput` accepted by create/update mutations; UI extends BackupSettings form with Schedule + Retention sections and shows `Schedule: ... — next ...` / `Retention: delete after N days — next check ...` rows in the destination list.
